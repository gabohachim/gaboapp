      ...
      builder: (context, snap) {
        ...
        return FutureBuilder<DateTime?>(
          ...
          builder: (context, lastSnap) {
            ...
            return FutureBuilder<bool>(
              future: BackupService.getLastCloudOk(),
              builder: (context, okSnap) {
                final cloudOk = okSnap.data ?? false;
                return FutureBuilder<DateTime?>(
                  future: BackupService.getLastCloudTime(),
                  builder: (context, cloudTimeSnap) {
                    final cloudTime = cloudTimeSnap.data;

                    final base = auto ? 'Auto respaldo activo' : 'Respaldo manual';
                    final lastLine = last == null ? 'Sin respaldos aún' : 'Último: ${BackupService.formatDateTime(last)}';
                    final cloudLine = cloudTime == null
                        ? 'Drive: sin sync'
                        : 'Drive: ${cloudOk ? 'OK' : 'falló'} (${BackupService.formatDateTime(cloudTime)})';
